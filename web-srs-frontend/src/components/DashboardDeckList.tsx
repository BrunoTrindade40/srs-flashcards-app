    ...
    <div className="flex flex-col gap-8">
      <div className="flex flex-col gap-4">
        <div className="flex justify-between items-center">
          <h2 className="text-xl font-bold text-slate-100">Seus Baralhos</h2>
        </div>
          ...
          <div className="flex flex-col gap-3">
            {activeDecks.map((deck) => {
              ...
              return (
                <div
                  key={deck.id}
                  className="flex flex-col md:flex-row justify-between items-start md:items-center bg-slate-900 border border-slate-800 p-5 rounded-2xl gap-4 hover:border-slate-700 transition-all shadow-sm hover:shadow-md"
                >
                  ...
                  <div className="flex gap-2 w-full md:w-auto justify-end">
                    <button
                      ...
                      className="flex-1 md:flex-none px-4 py-2 bg-slate-800 hover:bg-slate-700 text-slate-200 font-bold text-xs rounded-xl border border-slate-700 transition-all cursor-pointer shadow-sm"
                    >
                      Ver Detalhes
                    </button>
                    <button
        ...
        <div className="flex flex-col gap-3 pt-4 border-t border-slate-800">
          ...
          <div className="flex flex-col gap-2">
            {archivedDecks.map((deck) => (
              <div
                key={deck.id}
                className="flex justify-between items-center bg-slate-900/50 border border-slate-800 p-4 rounded-xl gap-4"
              >
                ...
                <button
                  onClick={() => navigate(`/deck/${deck.id}`)}
                  className="px-3 py-1.5 bg-slate-800 hover:bg-slate-700 text-slate-300 font-semibold text-xs rounded-lg border border-slate-700 transition-all cursor-pointer shadow-sm"
                >
                  Gerenciar
                </button>
              </div>
