...
export function ChaosStudyPage() {
  ...
  const { data, loading, error, refetch } = useQuery(GET_CHAOS_CARDS, {
    fetchPolicy: "network-only",
  });

  const [submitReview, { loading: submitting }] = useMutation(SUBMIT_REVIEW);

  const cards: FlashcardDue[] = data?.chaosCards || [];
  ...
  const handleRating = useCallback(
    async (rating: number) => {
      ...
      try {
        ...
        // Sustituído alert() nativo por Toast assíncrono
  ...
  if (cards.length === 0 || sessionFinished) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4 text-center max-w-md mx-auto px-4">
        <div className="w-16 h-16 rounded-full bg-amber-500/10 border border-amber-500/20 flex items-center justify-center text-amber-400 text-2xl mb-2">
          ⚡
        </div>
        ...
        <p className="text-slate-400 text-sm">
          Você revisou todas as cartas intercaladas agendadas para esta sessão.
        </p>
        <Link
          to="/dashboard"
          className="mt-2 px-6 py-2.5 bg-amber-500 hover:bg-amber-600 text-slate-950 font-semibold rounded-xl text-sm transition"
        >
          Voltar ao Dashboard
        </Link>
      </div>
